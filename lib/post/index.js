...
export const get404Page = () => {
  const notFoundData = fs.readdirSync(path.join("content/404"));
  const metaDataWithFrontMatter = fs.readFileSync(
    path.join("content/404", notFoundData[0]),
    "utf-8"
  );
  const { data: frontmatter, content } = matter(metaDataWithFrontMatter);
...
export const getBlogData = () => {
  const blogData = fs.readdirSync(path.join("content/posts"));
  const index = blogData.filter((page) => page.match(/^_/));
  const metaDataWithFrontMatter = fs.readFileSync(
    path.join("content/posts", index[0]),
    "utf-8"
  );
  const { data: frontmatter, content } = matter(metaDataWithFrontMatter);
