...
const Category = ({ value }) => {
  ...
  return (
    <>
      <SeoMeta title={filterCategory[0]} />
      <h1 className="pageTitle mt-16 md:mt-24">
        Showing Blog From{" "}
        <span className="text-primary">{filterCategory[0]}</span>
      </h1>
        ...
        {value.map((blog) => (
          <React.Fragment key={blog.slug}>
            <div className=" blog px-4" key={blog.slug}>
              ...
              <div className="h-auto md:h-60 h-37">
                <div className="text-center mb-4">
                  <h3>
                    <Link className="blogTitle" href={`/post/${blog.slug}`}>
